...
export const calculateAllNotifications = createAsyncThunk(
  ...
  async (_, { getState, dispatch }) => {
    ...
    const financings = state.financings?.list || [];
    ...
    matricules.forEach((matricule) => {
      ...
      if (techVisitDate) {
        ...
        if (isExpired || isExpiringSoon) {
          notifications.matricules.items.push({
          ...
          if (isCritical) { notifications.matricules.critical++; notifications.totalCriticalCount++; }
          else notifications.matricules.warning++;
        }
      }
    ...
    reservations.forEach((reservation) => {
      ...
      if (
        ...
        notifications.reservations.items.push({
        ...
        if (isCritical) { notifications.reservations.critical++; notifications.totalCriticalCount++; }
        else notifications.reservations.warning++;
      }
    });
    ...
    accidents.forEach((accident) => {
      ...
      if (daysSince !== null && daysSince <= 7 && accident.status !== "closed") {
        ...
        notifications.accidents.items.push({
        ...
        if (isCritical) { notifications.accidents.critical++; notifications.totalCriticalCount++; }
        else notifications.accidents.warning++;
      }
    });
    ...
    if (financings && financings.length > 0) {
      financings.forEach((financing) => {
        ...
        overdue.forEach((payment) => {
          notifications.payments.items.push({
        ...
        if (overdue.length === 0) {
          ...
          if (upcoming) {
            ...
            notifications.payments.total++;
            if (isCritical) { notifications.payments.critical++; notifications.totalCriticalCount++; }
            else notifications.payments.warning++;
          }
        }
      });
    }
...
const notificationSlice = createSlice({
  ...
  reducers: {
    ...
    closeNotificationPanel: (state) => { state.isOpen = false; },
    openNotificationPanel: (state) => { state.isOpen = true; },
